...
pub fn build(b: *std.Build) void {
    ...
    // Build output folder for generated example artifacts.
    const examples_out_dir = "build/_examples/sdl_gpu_triangle_wgsl";
    ...
    const example_triangle = b.addExecutable(.{
        .name = "example-triangle-wgsl",
        .root_module = b.createModule(.{
            .root_source_file = b.path("_examples/sdl_gpu_triangle_wgsl/src/main.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });
    ...
    // Match main exe rpath behavior.
    switch (target.result.os.tag) {
        .macos => example_triangle.root_module.addRPathSpecial("@executable_path"),
        .linux => example_triangle.root_module.addRPathSpecial("$ORIGIN"),
        else => {},
    }

    // Reuse any include/lib paths we configured for SDL3.
    if (vendored_sdl) {
        example_triangle.addIncludePath(.{ .cwd_relative = sdl_include_dir });
        example_triangle.addLibraryPath(.{ .cwd_relative = sdl_build_dir });
        if (sdl_ready_step) |s| example_triangle.step.dependOn(s);
    } else {
        if (sdl3_include_dir_opt.len != 0) addIncludePathIfExists(example_triangle, sdl3_include_dir_opt);
        if (sdl3_lib_dir_opt.len != 0) addLibraryPathIfExists(example_triangle, sdl3_lib_dir_opt);

        if (sdl3_include_dir_opt.len == 0 and sdl3_lib_dir_opt.len == 0) {
            if (sdl3_prefix_opt.len != 0) {
                addIncludePathIfExists(example_triangle, b.fmt("{s}/include", .{sdl3_prefix_opt}));
                addLibraryPathIfExists(example_triangle, b.fmt("{s}/lib", .{sdl3_prefix_opt}));
            } else {
                addIncludePathIfExists(example_triangle, b.pathResolve(&.{ "build", "_deps", "sdl3-src", "include" }));
                addLibraryPathIfExists(example_triangle, b.pathResolve(&.{ "build", "_deps", "sdl3-build" }));

                const prefixes = [_][]const u8{ "/opt/homebrew", "/usr/local" };
                inline for (prefixes) |p| {
                    addIncludePathIfExists(example_triangle, p ++ "/include");
                    addLibraryPathIfExists(example_triangle, p ++ "/lib");
                }
            }
        }
    }

    example_triangle.linkSystemLibrary("SDL3");

    b.installArtifact(example_triangle);

    const run_example_triangle_cmd = b.addRunArtifact(example_triangle);
    run_example_triangle_cmd.step.dependOn(b.getInstallStep());
    run_example_triangle_cmd.step.dependOn(example_triangle_shaders);

    const run_example_triangle_step = b.step("run-example-triangle", "Run the WGSL+naga SDL_gpu triangle example");
    run_example_triangle_step.dependOn(&run_example_triangle_cmd.step);
}
